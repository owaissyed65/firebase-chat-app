...
const Search = (props) => {
  ...
  const handleChat = async (users) => {
    ...
    try {
      ...
      if (!res.exists()) {
        ...
        let currentUserRef = await getDoc(
  ...
  return (
    <div>
      ...
      {error && (
        <>
          <div className="flex items-center w-full h-full justify-center mt-5 mb-2">
            User Not found
          </div>
      ...
      {load && (
        <div className="flex items-center w-full h-full justify-center mt-2">
          <Image src="/loader.svg" width={40} height={40} alt="load" />
        </div>
      )}
      {user && !load && (
        <>
